const { State, Pair } = require('../dist/futils').data;

describe('State', () => {
  it('should be able to construct State via of', () => {
    expect(State.of(1).run(0)).toBe(1);
    expect(State.of(1).exec(0)).toBe(0);
    expect(State['fantasy-land/of'](1).run(0)).toBe(1);
    expect(State['fantasy-land/of'](1).exec(0)).toBe(0);
  });

  it('should be able to construct State via constructors', () => {
    expect(State(s => Pair(s * 0.5, s)).run(2)).toBe(1);
    expect(State(s => Pair(s * 0.5, s)).exec(2)).toBe(2);
  });
  ...
  it('should be able to chain/flatMap', () => {
    ...
    expect(
      State.get()
        ['fantasy-land/chain'](x => State.of(x + 1))
        .run(1)
    ).toBe(2);
    expect(
      State.get()
  ...
  it('should be able to ap', () => {
    ...
    expect(
      State.of(1)
        ['fantasy-land/ap'](State.of(x => x + 1))
        .run(1)
    ).toBe(2);
    expect(
      State.of(1)
