...
describe('Either', () => {
  ...
  it('should be able to construct Right via of', () => {
    ...
    expect(Either['fantasy-land/of']('r').toString()).toBe('Right(r)');
  });
  ...
  it('should be able to construct an empty instance', () => {
    ...
    expect(Left.is(Either['fantasy-land/empty']())).toBe(true);
  });
  ...
  it('should be able to traverse', () => {
    ...
    expect(a['fantasy-land/traverse'](Array, x => [x]).join('')).toBe(
      'Right(1)'
    );
    expect(c.traverse(x => [x], Array).join('')).toBe('Left(1)');
    expect(c['fantasy-land/traverse'](Array, x => [x]).join('')).toBe(
      'Left(1)'
    );
  });
